...
No agent-to-agent interaction without hierarchy mediation. Soldiers cannot message each other directly. All communication flows: Soldier → Capo → Underboss → Don.

**Why:** Prevents shadow coordination where agents develop shared assumptions outside the spec.

**Violated when:** A Soldier asks another Soldier a question directly, or a Capo bypasses the Underboss.
...
An uncited claim is **stronzate** (bullshit) and must be flagged. The Consigliere has standing authority to invoke a Truth Check at any point. When flagged, the agent must either provide a citation or retract the claim.
...
Each Capo operates within a token budget set by the Underboss. Soldiers report resource consumption in their Tributes. If a territory exceeds its budget, work pauses until the Underboss approves more allocation. All results (merged code, passing tests) are reported up: Soldier → Capo → Underboss → Don.

**Violated when:** A Soldier dispatches without a budget, or a Capo proceeds after budget exhaustion.
...
**Violated when:** A Soldier "fixes" something that contradicts the spec without escalating.
