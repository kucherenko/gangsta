...
**Every phase gate requires Don approval.** Never skip a phase. **Never auto-advance** — except at two explicitly authorized sites:

1. `skills/the-sit-down/SKILL.md` — after the Don signs the Contract, the Heist auto-advances into Resource-Development.
2. `skills/the-hit/SKILL.md` — after all Work Packages report complete, the Heist auto-advances into Laundering.

The original rule — pause-and-prompt at each phase gate — is the default at every other transition.
